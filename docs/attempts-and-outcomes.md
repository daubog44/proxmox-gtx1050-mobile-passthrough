...
| Sunshine/Moonlight con GTX resa primaria | Con `AQ_DRM_DEVICES=card0:card1` e la cattura GBM su GTX, il monitor QEMU `Virtual-1` restava posseduto dalla VirtIO. L'importazione cross-GPU del DMA-BUF falliva (`Couldn't import RGB Image: 0000300C`), con frame corrotti o neri. Non era un errore del codec NVENC. | VirtIO (`card1`) è tornata primaria per Hyprland e per `adapter_name`, quindi cattura il proprio output. Sunshine usa comunque `h264_nvenc` sulla GTX: durante Moonlight `nvidia-smi pmon` ha mostrato il processo `sunshine` con `enc=4`. La configurazione, il motivo del binario bundled e i comandi di verifica sono in [sunshine-moonlight-omarchy.md](sunshine-moonlight-omarchy.md). |
