...
| `mstsc` Windows dopo cleanup | Le prove iniziali usavano NLA e davano `Could not find user in SAM database` / `SEC_E_NO_CREDENTIALS`. Con il profilo RDSTLS attuale il server arriva a `Sending server redirection`, ma il client chiude il primo tratto prima della prova visiva del secondo hand-over. | Il problema non e' xrdp, Flashback, GPU o passthrough. Il profilo RDSTLS e' necessario, ma il difetto residuo e' nella catena GNOME/Ubuntu: i repository ufficiali aggiornati non offrono ancora un fix per questa release. |
