...
install_key() {
  ...
  if [[ -n "${OMARCHY_SSH_PASSWORD:-}" && "${OMARCHY_ASKPASS_MODE:-}" != 1 ]]; then
    command -v sshpass >/dev/null || die 'OMARCHY_SSH_PASSWORD richiede sshpass nel flusso manuale'
    export SSHPASS="$OMARCHY_SSH_PASSWORD"
    ssh_command=(sshpass -e ssh)
  fi
  if [[ -n "${OMARCHY_SSH_PASSWORD:-}" ]]; then
    note 'autorizzo la chiave con la credenziale temporanea; non viene salvata'
  else
    note 'inserisci una sola volta la password SSH della VM per autorizzare la chiave dedicata del client Fedora'
  fi
  "${ssh_command[@]}" -T -o StrictHostKeyChecking=accept-new "$OMARCHY_USER@$OMARCHY_VM_HOST" "$remote_command"
